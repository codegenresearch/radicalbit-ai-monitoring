...
class BinaryClassificationDataQuality(DataQuality):
    ...
    feature_metrics: Optional[List[Union[NumericalFeatureMetrics, CategoricalFeatureMetrics]]] = None
    ...
    def validate_data(self):
        if self.n_observations is None:
            raise MetricsInternalError("n_observations is required")
        if self.class_metrics is None or len(self.class_metrics) == 0:
            raise MetricsInternalError("class_metrics cannot be empty")
        if self.feature_metrics is None or len(self.feature_metrics) == 0:
            raise MetricsInternalError("feature_metrics cannot be empty")
